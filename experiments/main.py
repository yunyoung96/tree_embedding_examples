...
from tree_utils import random_tree, count_nodes
...
def main():
    ...
    # 결과 디렉토리 생성
    result_dir = 'random_results'
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
        print(f"✓ 결과 디렉토리 생성: {result_dir}/")
    
    print("\n" + "="*80)
    print("🚀 트리 임베딩 방법 비교 분석 (랜덤 트리)")
    print("="*80)
    ...
    all_trees = [random_tree() for _ in range(20)]
    trees = all_trees[:150]  # 150개를 사용
    
    avg_nodes = np.mean([count_nodes(tree) for tree in trees])
    stage1_time = time.time() - stage1_start
    print(f"✓ {len(trees)}개 랜덤 트리 생성 완료 (평균 노드: {avg_nodes:.1f})")
    ...
    for i, r in enumerate(results, 1):
        save_heatmap(r, source='random', output_dir=result_dir)
        print(f"  [{i:2d}/{len(results)}] {r['name']:35s} → {result_dir}/random_{r['name'].replace('/', '_').replace(' ', '_')}.png")
    ...
    if bbdist_result:
        ...
        plot_filename = save_binary_branch_distance_plot(bbdist_result, source='random', output_dir=result_dir)
    ...
    print("⏱️  실행 시간 요약")
    print("="*80)
    print(f"  Stage 1 (트리 생성):      {stage1_time:>8.2f}초")
